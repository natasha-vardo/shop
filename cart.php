<?php 
    ...
    switch ($action) {
	    ...
	    case 'clear':
        $clear = mysqli_query($connect, "DELETE FROM cart WHERE cart_ip = '{$_SERVER['REMOTE_ADDR']}'");     
	    break;
        
        case 'delete':     
        $delete = mysqli_query($connect, "DELETE FROM cart WHERE cart_id = '$id' AND cart_ip = '{$_SERVER['REMOTE_ADDR']}'");        
        break;
    ...
    $result = mysqli_query($connect,"SELECT * FROM cart,table_products WHERE cart.cart_ip = '{$_SERVER['REMOTE_ADDR']}' AND table_products.products_id = cart.cart_id_products");
  ...
  if ( $_SESSION['auth'] != 'yes_auth' ) {
    ...
    echo '
       ...
       <li><label for="order_phone"><span>*</span>Телефон</label><input type="text" name="order_phone" id="order_phone" value="'.$_SESSION["order_phone"].'" /><span class="order_span_style" >Пример: 8 029 100 12 34</span></li>
        ...
        <li><label class="order_label_style" for="order_address"><span>*</span>Адрес<br /> доставки</label><input type="text" name="order_address" id="order_address" value="'.$_SESSION["order_address"].'" /><span>Пример: г. Минск,<br /> ул Интернациональная, д 1, кв 1</span></li>
    ';
}
    echo '
        <li><label class="order_label_style" for="order_note">Примечание</label><textarea name="order_note"  >'.$_SESSION["order_note"].'</textarea><span>Уточните информацию о заказе.<br /> Например, удоброе время для звонка <br />  нашего менеджера</span></li>
        </ul>
